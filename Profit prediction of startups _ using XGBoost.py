...
regressor = XGBRegressor(eta= 0.05, n_estimators= 100)
regressor.fit(X_train, y_train)

# Applying Grid Search to find the best model and the best parameters
from sklearn.model_selection import GridSearchCV
parameters = [{'eta': list(np.arange(0.01,0.101,0.01)), 'n_estimators': list(np.arange(50,151,50))}]
            #  Larger ranges were calculated for the parameters
            #  and finally reduced to this range for more accurate calculation and converged
...
output= [['XGBoost', -scores.mean()*100, scores.std()*100, average_differences[0], y_single_prediction[0][0]]]
