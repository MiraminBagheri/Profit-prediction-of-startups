...
regressor = RandomForestRegressor(bootstrap= True, max_depth= 50, max_features= None, min_samples_leaf= 1,
                                  min_samples_split= 2, n_estimators= 50, random_state = 0)
...
parameters = [{'n_estimators': [10, 50, 500],
                ...
                'max_features': [1, 4, "sqrt", "log2", None],
...
scores = cross_val_score(estimator = regressor, X = X_train, y = y_train.ravel(), cv = 10, scoring='neg_mean_squared_error')
...
output= [['Random Forest', -scores.mean()*100, scores.std()*100, average_differences[0], y_single_prediction[0][0]]]
